...
for (const viewport of [
  ...
  test(`${viewport.width}×${viewport.height} 页面与状态证据`, async ({
    ...
    await page
      ...
      .getByRole("button", { name: "模型权限", exact: true })
      .click();
    await expect(page).toHaveURL(/\/ai\/projects\/project-demo\/models$/);
    await expect(
      page.getByText("接口契约待接入", { exact: true }),
    ).toBeVisible();
...
test("新建项目跨页面管理密钥，返回后数据保留", async ({ page }) => {
  await enter(page, "/ai/projects");
  await page.getByRole("button", { name: "创建项目", exact: true }).click();
  await page.getByLabel("项目编码", { exact: true }).fill("NAVIGATION");
  await page.getByLabel("项目名称", { exact: true }).fill("跨页项目");
  await page.getByRole("button", { name: "保存", exact: true }).click();
  await page
    .locator("tr")
    .filter({ hasText: "跨页项目" })
  ...
  await page.getByLabel("Key 名称", { exact: true }).fill("跨页凭据");
  await page.getByRole("button", { name: "保存", exact: true }).click();
  await expect(page.getByLabel("一次性 Secret")).toBeVisible();
  await page.getByRole("button", { name: "我已保存，关闭" }).click();
  ...
  await page
    ...
    .getByRole("button", { name: "API Key", exact: true })
    .click();
  await expect(page.getByText("跨页凭据", { exact: true })).toBeVisible();
  await expect(page.getByLabel("一次性 Secret")).toHaveCount(0);
});
