...
const listPath = computed(() => {
  ...
  return String(
    route.meta.breadcrumbParentPath ?? productionProjectCostPath,
  );
});
...
const routeBase = computed(
  () =>
    ...
    String(route.meta.breadcrumbParentPath ?? productionProjectCostPath),
);
const activeMenuPath = computed(() => {
  if (
    route.path === productionProjectCostPath ||
    route.path.startsWith(`${productionProjectCostPath}/`) ||
    route.path === "/cost/production/new-production-cost" ||
    route.path.startsWith("/cost/production/new-production-cost/") ||
    route.path === "/new-production-cost" ||
    route.path.startsWith("/new-production-cost/") ||
    route.path === "/costmanagementnew/zc/projectcostnew" ||
    route.path.startsWith("/costmanagementnew/zc/projectcostnew/") ||
    route.path === "/costmanagementnew/zc/cost/production/project-cost" ||
    route.path.startsWith(
      "/costmanagementnew/zc/cost/production/project-cost/",
    )
  ) {
    return productionProjectCostPath;
  }
  if (typeof route.query.activeMenu === "string" && route.query.activeMenu) {
...
function patternHeaderGroup(patternName: string) { return groupedPatternHeaders.value.find((group) => group.patternName === patternName); }
...
function openPartHistory(row: CostBomPartItem) {
  router.push({
    ...
    query: {
      ...
      activeMenu: activeMenuPath.value,
    },
  });
}
