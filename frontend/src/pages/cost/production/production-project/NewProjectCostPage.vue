...
const pageTitle = "在产车型项目成本";
const productionProjectCostPath = "/cost/production/project-cost";
...
async function exportProjectCosts() {
  ...
  const task = await createCostBomExport(selected.projectId, {
...
async function openDetail(row: ProjectCostRow) {
  await router.push({
    ...
    query: {
      ...
      activeMenu: productionProjectCostPath,
    },
  });
}
...
async function openHistory(row: ProjectCostRow) {
  await router.push({
    path: `${productionProjectCostPath}/versions`,
    query: {
      projectId: String(row.projectId),
      versionId: String(row.id),
      version: resolveVersionParam(row),
    ...
    >
      ...
      <el-table-column label="操作" width="180" fixed="right" align="center">
        <template #default="{ row }">
          ...
          >
            <PermissionButton
                class="project-cost-page__gate-button"
                link
          ...
          <el-dropdown trigger="click">
            <el-button link type="primary" class="project-cost-more-trigger">
              更多
              <el-icon class="project-cost-page__dropdown-icon"><ArrowDown /></el-icon>
            </el-button>
            <template #dropdown>
              <el-dropdown-menu>
                <PermissionGuard
                    permission="costmanage:costbom:locking"
                >
                  <el-dropdown-item
                      :disabled="isPassGateActionDisabled(row)"
                      :style="{
    ...
    >
      <div class="project-cost-detail">
        ...
        <div class="project-cost-detail__toolbar">
          ...
          >
            新增
<!--            v-hasPermi="['costmanage:check:add']"-->
          </PermissionButton>
          <PermissionButton
          ...
          >
<!--            costmanage:check:pointreplication-->
            阀点复制
          </PermissionButton>
          <PermissionButton
