...
import {
  createCostBomExport,
  createCostBomVersionExportTask,
...
const pageTitle = computed(() => String(route.meta.title || "项目成本"));
...
async function queryProjectCosts(pageSize: number, pageNum: number) {
  ...
  const list = page.records
    .map((version) => toProjectCostRow(version, selectedProject()))
    .filter(matchesQuery);
...
function toProjectCostRow(
  ...
  return {
    ...
    projectCode:
      project?.projectCode ??
      readOptionalField(version, "projectCode") ??
      undefined,
    projectName:
...
async function exportProjectCosts() {
  ...
  const [startTime, endTime] = query.createdRange;
  const task = await createCostBomVersionExportTask({
    vehicleModelId: currentProjectId() || undefined,
    vehicleModelName:
      selectedProject()?.projectName || query.projectName || undefined,
    valveId: resolveValveId(query.gateCode),
    status: query.gateStatus
      ? mapGateStatusToApi(query.gateStatus)
      : undefined,
    version: query.bomVersion.trim() || undefined,
    createBy: query.creator.trim() || undefined,
    startTime: startTime || undefined,
...
async function deleteSelectedVersions() {
  ...
  BaseToast.success(`已删除 ${selectedRows.value.length} 个成本版本`);
  selectedRows.value = [];
  refreshProjectCosts();
}
...
<template>
  ...
  <PageContainer :title="pageTitle">
    ...
    >
      ...
      <el-table-column label="过阀状态" min-width="130">
        <template #default="{ row }">
          <BaseStatusTag
      ...
      <el-table-column label="锁定状态" min-width="100" align="center">
        <template #default="{ row }">
      ...
      <el-table-column prop="gateTime" label="过阀时间">
        <template #default="{ row }">
          {{ row.gateTime ?? "--" }}
        </template>
      </el-table-column>
      ...
      <el-table-column prop="updatedBy" label="更新人" min-width="130">
        <template #default="{ row }">
          {{ row.updatedBy || "--" }}
        </template>
      </el-table-column>
    ...
    <BaseFormDialog
      ...
      :confirm-button-props="{
        permission: 'cost:research:project-cost:list-page:detail-dialog-close',
      }"
    >
          ...
          >
            ...
            <el-tooltip v-if="index === 0" content="添加条件" placement="top">
              <PermissionButton
                ...
                plain
                type="primary"
                permission="cost:research:project-cost:list-page:detail-filter-add"
                :icon="Plus"
            ...
            <el-tooltip v-else content="删除条件" placement="top">
              <PermissionButton
                ...
                plain
                type="danger"
                permission="cost:research:project-cost:list-page:detail-filter-remove"
                :icon="Minus"
    ...
    <BaseFormDialog
      ...
      close-on-confirm
      :confirm-button-props="{
        permission: 'cost:research:project-cost:list-page:history-dialog-close',
      }"
    >
      <div class="project-cost-history__toolbar">
        <PermissionButton
          permission="cost:research:project-cost:list-page:history-compare"
          variant="primary"
          type="primary"
        ...
        <el-table-column label="操作" width="100" fixed="right" align="center">
          <template #default="{ row }">
            <PermissionButton
              link
              type="primary"
              permission="cost:research:project-cost:list-page:history-view-version"
    ...
    <BaseFormDialog
      ...
      close-on-confirm
      :confirm-button-props="{
        permission: 'cost:research:project-cost:list-page:compare-dialog-close',
      }"
    >
