...
const columnPermissionMap: Record<string, string> = {
  basic: "system:bom:basic", classify: "system:bom:category", "bom-extra": "system:bom:supplement",
  division: "system:bom:division", quantity: "system:bom:usage", target: "system:bom:target",
  "initial-eval": "system:bom:first:price", evaluation: "system:bom:assess", round: "system:bom:round",
  "meeting-price": "system:bom:meet", "srm-price": "system:bom:srm", "current-cost": "system:bom:current",
  "variant-target": "system:bom:target:pattern", "variant-current-no": "system:bom:current:pattern",
  "variant-current-yes": "system:bom:current:amortize:pattern", "variant-package": "system:bom:assess:pack",
  "variant-freight": "system:bom:assess:logi", other: "system:bom:version",
};
function hasColumnPermission(group: string): boolean {
  return hasPermi(columnPermissionMap[group] ?? group);
}
...
const summaryProjectName = computed(() => {
  ...
  try {
    ...
    // Ignore malformed legacy return paths.
  }
  return "--";
});
...
const columnAnchors = [
  ...
  { label: "初始评估值", value: "initial-eval" },
...
const staticColumnSettingChildrenMap: Record<string, ColumnSettingChild[]> = {
  ...
  "initial-eval": [
    { key: buildColumnChildKey("initial-eval", "col-1"), label: "初始评估值" },
  ],
  evaluation: [
    { key: buildColumnChildKey("evaluation", "col-1"), label: "评估值" },
    { key: buildColumnChildKey("evaluation", "col-2"), label: "出厂价" },
    { key: buildColumnChildKey("evaluation", "col-3"), label: "包装费" },
    { key: buildColumnChildKey("evaluation", "col-4"), label: "物流费" },
    { key: buildColumnChildKey("evaluation", "col-5"), label: "数据来源" },
  ],
  round: [
  ...
  "meeting-price": [
    ...
    {
      key: buildColumnChildKey("meeting-price", "col-7"),
      label: "定点会次(采购)",
    },
    { key: buildColumnChildKey("meeting-price", "col-8"), label: "出厂价" },
    { key: buildColumnChildKey("meeting-price", "col-9"), label: "包装费" },
    { key: buildColumnChildKey("meeting-price", "col-10"), label: "物流费" },
    {
    ...
    {
      key: buildColumnChildKey("meeting-price", "col-13"),
      label: "设计开发摊销",
    },
    { key: buildColumnChildKey("meeting-price", "col-14"), label: "入厂价" },
    {
    ...
    {
      key: buildColumnChildKey("meeting-price", "col-20"),
      label: "配额(采购)",
    },
    { key: buildColumnChildKey("meeting-price", "col-21"), label: "商务降本" },
    { key: buildColumnChildKey("meeting-price", "col-22"), label: "备注" },
  ],
  "srm-price": [
    {
  ...
  "current-cost": [
    { key: buildColumnChildKey("current-cost", "col-1"), label: "当前成本" },
    {
    ...
    { key: buildColumnChildKey("current-cost", "col-4"), label: "摊销数量" },
    { key: buildColumnChildKey("current-cost", "col-5"), label: "摊销金额" },
    { key: buildColumnChildKey("current-cost", "col-6"), label: "数据来源" },
    { key: buildColumnChildKey("current-cost", "col-7"), label: "备注" },
  ],
  other: [
...
function getPatternByPatternName(
  ...
  const patterns = row.costBomPattern ?? row.patterns ?? [];
  return patterns.find((p) => p.patternName === patternName);
}
  ...
  <PageContainer :title="pageTitle">
    ...
    <div class="cost-bom-detail-page">
      ...
      >
        ...
        <el-table-column
          type="index"
          :index="(index: number) => (index === 0 ? '' : index)"
          label="序号"
          width="64"
          fixed="left"
          align="center"
        ...
        />
        <el-table-column
          v-if="isColumnVisible('basic') && hasPermi('system:bom:basic')"
          label="基本信息"
          align="center"
          :label-class-name="anchorClass('basic')"
          :class-name="anchorClass('basic')"
        ...
        <el-table-column
          v-if="isColumnVisible('classify') && hasPermi('system:bom:category')"
          label="分类"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('bom-extra') && hasPermi('system:bom:supplement')"
          label="BOM补充信息"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('division') && hasPermi('system:bom:division')"
          label="分工"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('quantity') && hasPermi('system:bom:usage')"
          label="单车用量"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('target') && hasPermi('system:bom:target')"
          label="目标值"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('initial-eval') && hasPermi('system:bom:first:price')"
          label="初始评估值"
          align="center"
        ...
        >
          <el-table-column
            v-if="isColumnVisible(buildColumnChildKey('initial-eval', 'col-1'))"
        ...
        <el-table-column
          v-if="isColumnVisible('evaluation') && hasPermi('system:bom:assess')"
          label="评估值"
          align="center"
        ...
        >
          ...
          <el-table-column
            v-if="isColumnVisible(buildColumnChildKey('evaluation', 'col-2'))"
            prop="exFactoryPrice"
            label="出厂价"
            min-width="120"
            align="center"
          ...
          <el-table-column
            v-if="isColumnVisible(buildColumnChildKey('evaluation', 'col-3'))"
          ...
          <el-table-column
            v-if="isColumnVisible(buildColumnChildKey('evaluation', 'col-4'))"
          ...
          <el-table-column
            v-if="isColumnVisible(buildColumnChildKey('evaluation', 'col-5'))"
        ...
        <el-table-column
          v-if="isColumnVisible('round') && hasPermi('system:bom:round')"
          label="轮次报价"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('meeting-price') && hasPermi('system:bom:meet')"
          label="上会价"
          align="center"
        ...
        >
          ...
          <el-table-column
            v-if="
              isColumnVisible(buildColumnChildKey('meeting-price', 'col-22'))
            "
            prop="meetingRemark"
            label="备注"
            min-width="150"
        ...
        <el-table-column
          v-if="isColumnVisible('srm-price') && hasPermi('system:bom:srm')"
          label="SRM价格"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('current-cost') && hasPermi('system:bom:current')"
          label="当前成本"
          align="center"
        ...
        >
          ...
          >
            <template #default="{ row }">{{
              formatMoney(row.currentAmortize)
            }}</template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template #default="{ row }">
              {{ formatCostDataSource(row.currentDataSources) }}
            </template>
          </el-table-column>
          <el-table-column
        ...
        <el-table-column
          v-if="isColumnVisible('variant-target') && hasPermi('system:bom:target:pattern')"
        ...
        <el-table-column
          v-if="isColumnVisible('variant-current-no') && hasPermi('system:bom:current:pattern')"
        ...
        <el-table-column
          v-if="isColumnVisible('variant-current-yes') && hasPermi('system:bom:current:amortize:pattern')"
          label="各版型当前成本统计（含摊销）"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('variant-package') && hasPermi('system:bom:assess:pack')"
          label="各版型当前成本包装费统计"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('variant-freight') && hasPermi('system:bom:assess:logi')"
          label="各版型当前成本物流费统计"
          align="center"
        ...
        <el-table-column
          v-if="isColumnVisible('other') && hasPermi('system:bom:version')"
          label="其他"
          align="center"
      ...
      >
        <div class="cost-bom-detail-page__copy-valve">
          ...
          <el-alert
            ...
            :closable="false"
            show-icon
            :title="`将复制 ${sourceValveOptions.find((item) => String(item.valveId) === copyValveForm.sourceValveId)?.valveName || copyValveForm.sourceValveId} 阀点的零件信息到当前阀点 ${currentValveLabel || valveId}`"
          />
