...
async function handlePassGate(row: CostHistoryRow, command: PassGateCommand) {
  ...
  try {
    await openConfirm({
      title: "过阀确认",
      message: buildPassGateConfirmMessage(row, command),
      type: "warning",
      confirmText: "确定",
      cancelText: "取消",
    });
...
function buildPassGateConfirmMessage(
  ...
  const targetName = `${row.vehicleModel || row.projectName || "--"}（${
    row.valvePoint || "--"
  }）`;
  const targetStatus = passGateSuccessText[command].replace(/^已设为|^已/, "");
...
async function exportSelectedHistory() {
  ...
  const task = await createCostBomExport(undefined, {
    versionId,
    version: resolveHistoryVersionParam(selected),
    format: "xlsx",
  });
  latestTaskFileName.value = buildCostBomExportFileName({
    projectName: selected.vehicleModel || selected.projectName,
...
<!--          </el-form-item>-->
    ...
    <BaseImportDialog
      ...
      :confirm-button-props="{
        permission: `${listPagePermissionPrefix}:import`,
      }"
      @import="handleImport"
