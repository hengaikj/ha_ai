...
// 页面定位分组键与后端列权限分组键存在少量命名差异。
const columnPermissionKeyMap: Record<string, string> = {
  basic: "basic",
  classify: "category",
  "bom-extra": "supplement",
  division: "division",
  quantity: "usage",
  target: "target",
  evaluation: "evaluation",
  "meeting-price": "meeting-price",
  "srm-price": "srm-price",
  "current-cost": "current",
  "variant-target": "variant-target",
  "variant-current-no": "variant-current-no",
  "variant-current-yes": "variant-current-yes",
  "variant-package": "variant-package",
  "variant-freight": "variant-freight",
  other: "other",
};

function hasColumnPermission(group: string): boolean {
  const permissionKey = columnPermissionKeyMap[group] ?? group;
  return hasPermi(columnPermission(permissionKey));
}
...
const historyListPath = "/costmanagementnew/zy/costbom";

const listPath = computed(() => historyListPath);

const routeBase = computed(() => "/cost/research/new-history");
...
const summaryValveName = computed(() => route.query.valveName || "");
...
function getPatternValue(
  ...
  return pattern ? (pattern as Record<string, unknown>)[field] : undefined;
}
...
function operatorsForCondition(
  ...
  const fieldType = field?.fieldType || field?.dataType || "string";
  const configuredOperators = filterOperatorConfig.value[fieldType];
...
async function loadFilterMetadata() {
  try {
    ...
    console.log("[成本履历筛选] filterResponse 原始数据:", filterResponse);
    ...
    console.log("[成本履历筛选] 解析后的 rows:", rows);
    ...
    console.log("[成本履历筛选] 最终 dynamicGroupAttrMap:", groupMap);
    console.log("[成本履历筛选] 最终 boardOptions:", boardOptions.value);
  } catch (error) {
    console.error("[成本履历筛选] 加载筛选元数据失败:", error);
...
async function loadDetailParts() {
  ...
  try {
    const conditions = detailConditions.value
      ...
      .map((item) => {
        const field = resolvedFilterFields().find(
          (candidate) => candidate.value === item.attributeName,
        );
        return {
          logic: item.logic,
          ...
          field: field?.field || item.attributeName,
          fieldType: field?.fieldType || field?.dataType || "string",
...
function formatPrice3(value: unknown, len: number) {
...
const columnSettingOptions = computed(() =>
  columnAnchors
    .filter((item) => hasColumnPermission(item.value))
    .map((item) => ({
      key: item.value,
      label: item.label,
      ...
      >
        ...
        >
          ...
          >
            ...
            >
              <template #default="{ row }">
                {{ (row as any).costBomExtend?.amortizationBaseCount }}
              </template>
            </el-table-column>
          </el-table-column>
        ...
        >
          ...
          >
            <template #default="{ row }">
              {{ (row as any).costBomExtend?.summaryRemark }}
            </template>
          </el-table-column>
        </el-table-column>
...
:global(.page-container.is-page-fullscreen .cost-bom-detail-page) {
  height: 100%;
  min-height: 0;
}

:global(.page-container.is-page-fullscreen .cost-bom-detail-page__table) {
  flex: 1 1 0;
  min-height: 0;
}
